// Idempotent dev seed for the Final-Task validation scenario:
//   2 orgs, 4 users, memberships (owner/editor/viewer across orgs),
//   one Org-A workflow with llm_call + conditional_branch + approval_gate
//   and a manual trigger.
// Uses the nhost auth signup endpoint for users and the admin GraphQL
// endpoint for everything else. Safe to re-run.
import fs from 'node:fs';

const GRAPHQL = 'https://local.hasura.local.nhost.run/v1/graphql';
const AUTH = 'https://local.auth.local.nhost.run/v1/signup/email-password';
